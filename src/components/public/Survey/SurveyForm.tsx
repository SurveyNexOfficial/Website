		...
		<div className="max-w-7xl mx-auto w-full">
			...
			<div className="mb-8">
				<div className="flex items-center">
					...
					<div className={`flex-1 h-1 mx-4 ${currentStep !== "basic" ? "bg-black" : "bg-gray-200"}`}></div>
					<div
						className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
					...
					<div className={`flex-1 h-1 mx-4 ${currentStep === "preview" ? "bg-black" : "bg-gray-200"}`}></div>
					<div
						className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${
			...
			<form onSubmit={handleSubmit(onSubmit)}>
				...
				{currentStep === "basic" && (
					<div className="flex flex-col gap-y-6">
						...
						<div>
							<label className="block text-sm font-medium text-black mb-2">Terms and Conditions *</label>
							<textarea
											...
											{watchedQuestions[questionIndex]?.responseOptions?.map(
												(option, optionIndex) => (
													<div key={optionIndex} className="flex items-center gap-2">
														...
														/>
														{(watchedQuestions[questionIndex]?.responseOptions?.length ||
															0) > 1 && (
															<button
																type="button"
																onClick={() =>
														...
														<label
															...
															<input
																...
																checked={
																	watchedQuestions[
																		questionIndex
																	]?.allowedMediaTypes?.includes(mediaType.value) ||
																	false
																}
																onChange={() =>
					...
					<div className="space-y-8">
						<div className="bg-gray-50 rounded-sm p-6">
							...
							<div className="space-y-4 text-sm">
								...
								<div>
									<span className="font-medium">Questions:</span> {watchedForm.questions?.length || 0}
								</div>
							</div>
						</div>
